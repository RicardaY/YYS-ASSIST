...
int yys_function::Tips(Game *game) {
	Log(_T("  3.需关闭帧率显示\r\n"), game);
	Log(_T("  2.需使用简约战斗主题\r\n"), game);
	Log(_T("  1.若是使用mumu模拟器，桌面右键mumu-属性-兼容性-设置win7兼容\r\n"), game);
	Log(_T("  ****小提示****\r\n"), game);
	return 0;
}
...
bool yys_function::IsInYard(snow *sn, Game *game) {
	...
	while (i > 0)
	{
		...
		if ((sn->FindStrFast(269, 25, 331, 68, _T("加成"), _T("443B31-443B32"), 0.8, &intX, &intY) != -1) || (sn->FindStrFast(230, 40, 850, 200, _T("探|索"), _T("1D1D0C-1E1D0C"), 0.85, &intX, &intY) != -1) || (sn->FindStrFast(230, 40, 850, 200, _T("探|索"), _T("493B22-493C23"), 0.85, &intX, &intY) != -1) || (sn->FindStrFast(230, 40, 850, 200, _T("探|索"), _T("25323C-120E10"), 0.8, &intX, &intY) != -1))
		{
			isFind = 1;
			Log(_T("已在庭院界面...\r\n"), game);
			break;
	...
	while (1)
	{
		...
		if (targetSite <= currentSite){
			if (mode == _T("findstr") && sn->FindStrFast(startXY.x, startXY.y, endXY.x, endXY.y, tartgetPicFont, color_format, sim, &intX, &intY) != -1){
				...
				DelayPlus(game, sn, 500, 800);
				sn->DragMouse(startx, starty, startx, endy);
				DelayPlus(game, sn, 500, 800);
			}
		}//上移
			...
			if (mode == _T("findstr") && sn->FindStrFast(startXY.x, startXY.y, endXY.x, endXY.y, tartgetPicFont, color_format, sim, &intX, &intY) != -1){
				...
				sn->EnableRealMouse(1, 20, 30);
				DelayPlus(game, sn, 500, 800);
				sn->DragMouse(startx, endy, startx, starty);
				DelayPlus(game, sn, 500, 800);
			}
		}
		DelayPlus(game, sn, 500, 800);
...
int yys_function::EnterEvent(snow *sn, Game *game, bool isReadyClick) {
	...
	while (1)
	{
		...
		if ((isReturnAsExitCondition == 1) && (sn->FindPic(335, 256, 616, 497, _T("魂.bmp|魂(底部).bmp|魂(氪金).bmp|魂底(氪金).bmp"), _T("202020"), 0.9, 0, &intX, &intY) == -1)) {
			Log(_T("结算成功...\r\n"), game);
	...
	if (jc_category == _T("御魂"))
	{
		if (jc_index == _T("1"))
		{
			...
			while (1)
			{
				//自动拒绝悬赏
				if (AutoRejectXS(sn, game) == EXIT_THREAD) { return EXIT_THREAD; }
				if (sn->FindStrFast(start_xy.x, start_xy.y, end_xy.x, end_xy.y, _T("加成"), _T("443B31-443B32"), 0.8, &intX, &intY) != -1)
				{
					Log(_T("点击加成灯笼...\r\n"), game);
					if (DelayPlus(game, sn, 500, 1000) == EXIT_THREAD) { return EXIT_THREAD; }
					CPoint pos = sn->RandomOffsetPos(intX, intY, 0, 5);
...
int yys_function::returnYard(snow *sn, Game *game) {
	...
	while (true)
	{
		...
		//找返回
		exitRes1 = sn->FindPic(0, 0, 217, 107, _T("返回1.bmp|返回2.bmp|返回3.bmp|返回4.bmp|返回5.bmp|返回6.bmp|返回7.bmp|返回8.bmp|退出.bmp|退出2.bmp"), _T("202020"), 0.85, 0, &intX, &intY);
		if (exitRes1 != -1)
		{
			if (DelayPlus(game, sn, 200, 300) == EXIT_THREAD) { return EXIT_THREAD; }
			CPoint pos = sn->RandomOffsetPos(intX, intY);
		...
		//创建队伍退出 case3：退出邀请面板
		exitRes1 = sn->FindPic(512, 396, 649, 468, _T("邀请.bmp"), _T("202020"), 0.85, 0, &intX, &intY);
		exitRes2 = sn->FindPic(303, 397, 448, 465, _T("邀请取消.bmp"), _T("202020"), 0.85, 0, &intX, &intY);
		...
		//探索退出
		exitRes1 = sn->FindPic(299, 271, 457, 337, _T("探索取消.bmp"), _T("202020"), 0.85, 0, &intX, &intY);
		exitRes2 = sn->FindPic(505, 274, 656, 334, _T("探索确认.bmp"), _T("202020"), 0.85, 0, &intX, &intY);
		if ((exitRes1 != -1) && (exitRes2 != -1))
		{
			if (DelayPlus(game, sn, 200, 300) == EXIT_THREAD) { return EXIT_THREAD; }
			CPoint pos = sn->RandomOffsetPos(intX, intY);
		...
		//创建队伍退出 case1：取消创建队伍
		exitRes1 = sn->FindPic(598, 406, 714, 481, _T("创建.bmp"), _T("202020"), 0.85, 0, &intX, &intY);
		exitRes2 = sn->FindPic(239, 408, 363, 481, _T("取消.bmp"), _T("202020"), 0.85, 0, &intX, &intY);
		if ((exitRes1 != -1) && (exitRes2 != -1))
		{
			if (DelayPlus(game, sn, 200, 300) == EXIT_THREAD) { return EXIT_THREAD; }
			CPoint pos = sn->RandomOffsetPos(intX, intY);
		...
		//创建队伍退出 case2：退出组队面板
		if (sn->FindStrFast(542, 308, 594, 339, _T("确定"), _T("4D3A21-4E3B22"), 0.9, &intX, &intY) != -1)
		{
			if (DelayPlus(game, sn, 200, 300) == EXIT_THREAD) { return EXIT_THREAD; }
			CPoint pos = sn->RandomOffsetPos(intX, intY);
			sn->MoveClick(pos.x, pos.y);
		...
		if (sn->FindStrFast(528, 303, 587, 332, _T("确认"), _T("4D3A21-4E3B22"), 0.9, &intX, &intY) != -1) {
			if (DelayPlus(game, sn, 200, 300) == EXIT_THREAD) { return EXIT_THREAD; }
			CPoint pos = sn->RandomOffsetPos(intX, intY);
			sn->MoveClick(pos.x + 5, pos.y + 5);
			if (DelayPlus(game, sn, 200, 300) == EXIT_THREAD) { return EXIT_THREAD; }
