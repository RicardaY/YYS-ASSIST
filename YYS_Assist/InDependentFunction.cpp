...
int InDependentFunction::AutoPVP(snow *sn, Game *game) {
	...
	while (1) {
		...
		if (sn->FindPic(248, 12, 450, 173, _T("失败.bmp"), _T("202020"), 0.8, 0, &intX, &intY) != -1) {//
			if (DelayPlus(game, sn, 1800, 2200) == EXIT_THREAD) { return EXIT_THREAD; }
			Log(_T("挑战失败...\r\n"), game);
			if (sn->FindPic(364, 424, 488, 474, _T("上限.bmp"), _T("202020"), 0.9, 0, &intX, &intY) != -1) {
...
int InDependentFunction::AutoPVPToMingShi(snow *sn, Game *game) {
	...
	while (1) {
		...
		if (sn->FindPic(410, 186, 502, 271, _T("段位上升.bmp"), _T("202020"), 0.9, 0, &intX, &intY) != -1) {
			if (DelayPlus(game, sn, 1800, 2200) == EXIT_THREAD) { return EXIT_THREAD; }
			Log(_T("段位上升...\r\n"), game);
...
int InDependentFunction::StartNewEvent(int round, int mode, int flag, int bossflag, snow *sn, Game *game) {
	...
	sn->UseDict(6);
	//初始化活动活动名/体力名
	CString activityName = _T("祈舞唤神--宴场维和");
	CString activitySubName = _T("宴场维和");
	CString TiLiName = _T("青羽帖");
	//定义局部变量
	long intX, intY;
	Log(_T("开始前请进入") + activityName  + _T("界面(并配置好队伍，自己锁定阵容)...\r\n"), game);
	if(mode)
		Log(_T("检测到使用体力挑战，请手动切换到体力...\r\n"), game);
	else
		Log(_T("检测到使用") + TiLiName  + _T("挑战，请手动切换到")+ TiLiName + _T("...\r\n"), game);
	int count = 0;
	CString logmsg;
	...
	while (count < round) {
		//自动拒绝悬赏
		if (AutoRejectXS(sn, game) == EXIT_THREAD) { return EXIT_THREAD; }
		if (!mode&&sn->FindPic(651, 17, 758, 44, _T("活动0体.bmp"), _T("202020"), 0.9, 0, &intX, &intY) != -1) {
			count = round;
			Log(TiLiName + _T("用尽...\r\n"), game);
			break;
		}
		while (1) {
			if (AutoRejectXS(sn, game) == EXIT_THREAD) { return EXIT_THREAD; }
			//6E4745-252E2C
			if (sn->FindStrFast(855, 459, 917, 495, _T("活动挑战"), _T("363538-363639"), 0.85, &intX, &intY) != -1) {  //363538-363639
				if (DelayPlus(game, sn, 500, 800) == EXIT_THREAD) { return EXIT_THREAD; }
				Log(_T("点击挑战...\r\n"), game);
				CPoint pos = sn->RandomOffsetPos(intX, intY, 5, 10);
				sn->MoveClick(pos.x, pos.y);
		...
		while (1) {
			...
			if (flag && !markflag && sn->FindPic(143, 163, 220, 354, _T("活动标记.bmp"), _T("202020"), 0.9, 0, &intX, &intY) == -1) {
				if (DelayPlus(game, sn, 500, 1000) == EXIT_THREAD) { return EXIT_THREAD; }//198,212,266,323,
				Log(_T("标记式神(左1)...\r\n"), game);
				CPoint pos = sn->RandomOffsetPos(194, 352, 1, 3);
				sn->MoveClick(194, pos.y);
				if (DelayPlus(game, sn, 500, 800) == EXIT_THREAD) { return EXIT_THREAD; }
			}
			else if (flag && !markflag && sn->FindPic(143, 163, 220, 354, _T("活动标记.bmp"), _T("202020"), 0.9, 0, &intX, &intY) != -1) {
				Log(_T("已标记我方式神...\r\n"), game);
				markflag = 1;
			}
			////标记boss
			...
			if (sn->FindStrFast(855, 459, 917, 495, _T("活动挑战"), _T("363538-363639"), 0.85, &intX, &intY) != -1) {
				markflag = 0;
				//markflag2 = 0;
				break;
			}
...
int InDependentFunction::DIYFunction(int ready, int clearing, int whitecard, int challenge, snow *sn, Game *game) {
	...
	while (1) {
		...
		//抽白票
		if (whitecard) {
			if (sn->FindPic(183, 464, 259, 511, _T("厕纸.bmp"), _T("202020"), 0.9, 0, &intX, &intY) != -1) {
				Log(_T("十连抽取...\r\n"), game);
				CPoint pos = sn->RandomOffsetPos(intX, intY, 5, 10);
				if (DelayPlus(game, sn, 500, 800) == EXIT_THREAD) { return EXIT_THREAD; }
				sn->MoveClick(pos.x, pos.y);
