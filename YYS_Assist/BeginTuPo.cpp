...
int BeginTuPo::StartToFinish(int count, int clickx, int clicky, snow *sn, Game *game) {
	...
	while (1) {
		...
		if (sn->FindPic(0, 0, 56, 58, _T("退出(透明).bmp|退出1.bmp|退出.bmp"), _T("202020"), 0.9, 0, &intX, &intY) != -1) {
...
int BeginTuPo::isHaveTicket(snow *sn, Game *game) {
	...
	while (i > 0) {
		//自动拒绝悬赏
		if (AutoRejectXS(sn, game) == EXIT_THREAD) { return EXIT_THREAD; }
		if ((sn->FindPic(867, 6, 886, 37, _T("突破券0.bmp"), _T("202020"), 0.95, 0, &intX, &intY) != -1) && (sn->FindPic(858, 2, 885, 37, _T("突破券10.bmp"), _T("202020"), 0.95, 0, &intX, &intY) == -1)) {
			if (DelayPlus(game, sn, 200, 500) == EXIT_THREAD) { return EXIT_THREAD; }
			return 0;
		}
		if (DelayPlus(game, sn, 200, 300) == EXIT_THREAD) { return EXIT_THREAD; }
...
int BeginTuPo::ExitDo(int clickx, int clicky, snow *sn, Game *game) {
	...
	while (1) {
		if (AutoRejectXS(sn, game) == EXIT_THREAD) { return EXIT_THREAD; }
		//退出位置 0, 0, 56, 58
		if ((flag == 0) && (sn->FindPic(0, 0, 56, 58, _T("退出(透明).bmp|退出1.bmp|退出.bmp"), _T("202020"), 0.9, 0, &intX, &intY) != -1)) {
		...
		if (sn->FindStrFast(531, 303, 584, 333, _T("确认"), _T("4D3A21-4E3B22"), 0.9, &intX, &intY) != -1) {
			if (DelayPlus(game, sn, 300, 800) == EXIT_THREAD) { return EXIT_THREAD; }
...
int BeginTuPo::GetGTPos(CArray<CPoint, CPoint>& rArrString, snow *sn, Game *game) {
	...
	postList.RemoveAll();
	CString poscount = sn->FindPicEx(282, 92, 874, 418, _T("突破地方.bmp|突破地方1.bmp"), _T("202020"), 0.95, 0);
	if (poscount.IsEmpty()) {
		return 0;
	}
	if (!poscount.IsEmpty()) {
...
int BeginTuPo::GetLTPos(CArray<CPoint, CPoint>& rArrString, snow *sn, Game *game) {
	...
	CStringArray arrayList, postList, tempList;
	postList.RemoveAll();
	CString poscountL = sn->FindPicEx(527, 84, 564, 493, _T("突破地方.bmp"), _T("202020"), 0.9, 0);
	CString poscountR = sn->FindPicEx(785, 96, 824, 493, _T("突破地方.bmp"), _T("202020"), 0.9, 0);
