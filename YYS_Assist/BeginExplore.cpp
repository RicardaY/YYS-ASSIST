...
int BeginExplore::SingleChooseLayer(int layer, CPoint startXY, CPoint endXY, snow *sn, Game *game) {
	...
	while (1) {
		if (AutoRejectXS(sn, game) == EXIT_THREAD) { return EXIT_THREAD; }
		if (sn->FindPic(642, 367, 773, 438, _T("探索按钮.bmp"), _T("202020"), 0.9, 0, &intX, &intY) != -1) {
			Log(_T("已选择...\r\n"), game);
			break;
		}
...
int BeginExplore::ChooseDiff(int diff, snow *sn, Game *game) {
	...
	else if (!diff)
	{
		while (1)
		{
			...
			else if (sn->FindPic(661, 379, 758, 425, _T("探索按钮.bmp"), _T("202020"), 0.85, 0, &intX, &intY) != -1) {
				if (DelayPlus(game, sn, 1500, 2000) == EXIT_THREAD) { return EXIT_THREAD; }
				if (sn->FindPic(442, 386, 537, 425, _T("组队按钮.bmp"), _T("202020"), 0.85, 0, &intX, &intY) == -1) {
					Log(_T("已选择普通难度...\r\n"), game);
					break;
				}
			}
...
int BeginExplore::findCommonMonster(int speed, CString progressBar, CString materialKinds, snow *sn, Game *game) {
	if (sn == NULL)
	{
		return 0;
	}
	...
	if(IsChangeMaterial(sn, game) == 0)
	{
		...
		while (1)
		{
			...
			if (sn->FindStrFast(541, 310, 596, 339, _T("确定"), _T("4D3A21-4E3B22"), 0.9, &intX, &intY) != -1) {
				if (DelayPlus(game, sn, 500, 800) == EXIT_THREAD) { return EXIT_THREAD; }
				CPoint pos = sn->RandomOffsetPos(intX, intY, 0, 5);
				sn->MoveClick(pos.x, pos.y);
				if (DelayPlus(game, sn, 500, 800) == EXIT_THREAD) { return EXIT_THREAD; }
...
int BeginExplore::findBossMonster(int speed, CString progressBar, CString materialKinds, snow *sn, Game *game) {
	if (sn == NULL)
	{
		return 0;
	}
...
int BeginExplore::InnerCreatTeamToInviteUpper(int preferArea, snow *sn, Game *game) {
	...
	while (1)
	{
		//自动拒绝悬赏
		if (AutoRejectXS(sn, game) == EXIT_THREAD) { return EXIT_THREAD; }
		if (sn->FindPic(741, 443, 875, 498, _T("创建队伍.bmp"), _T("202020"), 0.85, 0, &intX, &intY) != -1)
		{
			Log(_T("创建队伍...\r\n"), game);
			if (DelayPlus(game, sn, 1000, 1500) == EXIT_THREAD) { return EXIT_THREAD; }
			CPoint pos = sn->RandomOffsetPos(intX, intY, 0, 5);
		...
		if (sn->FindPic(603, 412, 712, 475, _T("创建.bmp"), _T("202020"), 0.9, 0, &intX, &intY) != -1) {
			break;
		}
		if (DelayPlus(game, sn, 200, 500) == EXIT_THREAD) { return EXIT_THREAD; }
	}
	...
	while (1)
	{
		if (sn->FindPic(603, 412, 712, 475, _T("创建.bmp"), _T("202020"), 0.85, 0, &intX, &intY) != -1)
		{
			if (DelayPlus(game, sn, 500, 800) == EXIT_THREAD) { return EXIT_THREAD; }
			CPoint pos = sn->RandomOffsetPos(intX, intY, 0, 5);
			sn->MoveClick(pos.x + 5, pos.y + 2);
...
int BeginExplore::InnerCreatTeamToInviteLower(int preferArea, snow *sn, Game *game) {
	...
	while (1)
	{
		if (sn->FindPic(536, 407, 632, 454, _T("邀请.bmp"), _T("202020"), 0.9, 0, &intX, &intY) == -1) {
			if (sn->FindPic(788, 159, 846, 221, _T("加.bmp"), _T("202020"), 0.9, 0, &intX, &intY) != -1)//探索加
			{
				CPoint pos = sn->RandomOffsetPos(intX, intY, 0, 5);
		...
		if (sn->FindPic(353, 61, 434, 109, _T("好友(选中).bmp|好友(未选中).bmp"), _T("151515"), 0.85, 0, &intX, &intY) != -1) {
			break;
		}
		if (DelayPlus(game, sn, 200, 500) == EXIT_THREAD) { return EXIT_THREAD; }
	}
	...
	while (1)
	{
		...
		{
			if (sn->FindPic(353, 61, 434, 109, _T("好友(未选中).bmp"), _T("151515"), 0.85, 0, &intX, &intY) != -1)
			{
				Log(_T("选择同区好友...\r\n"), game);
				if (DelayPlus(game, sn, 500, 800) == EXIT_THREAD) { return EXIT_THREAD; }
				CPoint pos = sn->RandomOffsetPos(intX, intY, 0, 5);
			...
			if (sn->FindPic(353, 61, 434, 109, _T("好友(选中).bmp"), _T("202020"), 0.85, 0, &intX, &intY) != -1) {
				break;
			}
		}
		else if ((preferArea == 1) || (preferArea == 4))
		{
			if (sn->FindPic(440, 68, 516, 103, _T("跨区(未选中).bmp"), _T("202020"), 0.85, 0, &intX, &intY) != -1)
			{
				Log(_T("选择跨区好友...\r\n"), game);
				if (DelayPlus(game, sn, 500, 800) == EXIT_THREAD) { return EXIT_THREAD; }
				CPoint pos = sn->RandomOffsetPos(intX, intY, 0, 5);
	...
	if ((preferArea == 0) || (preferArea == 1)) {
		...
		while (1)
		{
			...
			if ((sn->FindPic(874, 436, 953, 518, _T("挑战.bmp|挑战(未进队).bmp"), _T("202020"), 0.95, 0, &intX, &intY) != -1) && (sn->FindPic(536, 407, 632, 454, _T("邀请.bmp"), _T("202020"), 0.9, 0, &intX, &intY) == -1)) {
				break;
			}
			if (DelayPlus(game, sn, 200, 500) == EXIT_THREAD) { return EXIT_THREAD; }
		}
	...
	if ((preferArea == 3) || (preferArea == 4)) {
		...
		while (1)
		{
			if (sn->SetPath(_T("fixTeamer")) && sn->FindPic(319, 138, 692, 316, sn->MatchPicName(_T("teamer?.bmp")), _T("202020"), 0.9, 0, &intX, &intY) != -1) {
				if (DelayPlus(game, sn, 500, 800) == EXIT_THREAD) { return EXIT_THREAD; }
				CPoint pos = sn->RandomOffsetPos(intX, intY, 0, 5);
...
int BeginExplore::EnterExplore(int speed, CString progressBar, CString materialKinds, snow *sn, Game *game) {
	if (sn == NULL)
	{
		return 0;
	}
	...
	while (1)
	{
		//自动拒绝悬赏
		if (AutoRejectXS(sn, game) == EXIT_THREAD) { return EXIT_THREAD; }
		int resultBoss = findBossMonster(speed, progressBar, materialKinds, sn, game);
		if (resultBoss == 0)
		{
			int resultCommon = findCommonMonster(speed, progressBar, materialKinds, sn, game);
...
int BeginExplore::ExitExploreEvent(snow *sn, Game *game) {
	...
	while (1)
	{
		...
		//使用字库
		sn->UseDict(0);
		if (sn->FindStrFast(523, 286, 640, 317, _T("确认"), _T("4D3A21-4E3B22"), 0.9, &intX, &intY) != -1) {
			if (DelayPlus(game, sn, 500, 800) == EXIT_THREAD) { return EXIT_THREAD; }
		...
		if (sn->FindPic(427, 359, 788, 465, _T("组队按钮.bmp|探索按钮.bmp"), _T("202020"), 0.9, 0, &intX, &intY) != -1) {
			break;
		}
		if (sn->FindPic(74, 164, 136, 219, _T("接受邀请.bmp"), _T("202020"), 0.9, 0, &intX, &intY) != -1) {
			break;
		}
		if (sn->FindStrFast(269, 25, 331, 68, _T("加成"), _T("443B31-443B32"), 0.8, &intX, &intY) != -1)
		{
			break;
		}
		//换字库
...
//判断是否需要换狗粮
int BeginExplore::IsChangeMaterial(snow *sn, Game *game) {
	if (sn == NULL)
	{
		return 0;
	}
...
int BeginExplore::SelectProgressBar(CString progress, snow *sn, Game *game) {
	...
	if (progress == _T("0")) {
		if (sn->FindPic(80, 506, 739, 539, _T("进度条.bmp"), _T("202020"), 0.9, 0, &intX, &intY) != -1) {
			if (DelayPlus(game, sn, 500, 800) == EXIT_THREAD) { return EXIT_THREAD; }
			sn->DragMouse(intX + 15, intY, intX + 45, intY);
			if (DelayPlus(game, sn, 500, 800) == EXIT_THREAD) { return EXIT_THREAD; }
		}
	}
		...
		if (sn->FindPic(80, 506, 739, 539, _T("进度条.bmp"), _T("202020"), 0.9, 0, &intX, &intY) != -1) {
			if (DelayPlus(game, sn, 500, 800) == EXIT_THREAD) { return EXIT_THREAD; }
			sn->DragMouse(intX + 15, intY, intX + 75, intY);
			if (DelayPlus(game, sn, 500, 800) == EXIT_THREAD) { return EXIT_THREAD; }
		}
	}
		...
		if (sn->FindPic(80, 506, 739, 539, _T("进度条.bmp"), _T("202020"), 0.9, 0, &intX, &intY) != -1) {
			if (DelayPlus(game, sn, 500, 800) == EXIT_THREAD) { return EXIT_THREAD; }
			sn->DragMouse(intX + 15, intY, intX + 105, intY);
			if (DelayPlus(game, sn, 500, 800) == EXIT_THREAD) { return EXIT_THREAD; }
		}
	}
		...
		if (sn->FindPic(80, 506, 739, 539, _T("进度条.bmp"), _T("202020"), 0.9, 0, &intX, &intY) != -1) {
			if (DelayPlus(game, sn, 500, 800) == EXIT_THREAD) { return EXIT_THREAD; }
			sn->DragMouse(intX + 15, intY, intX + 135, intY);
			if (DelayPlus(game, sn, 500, 800) == EXIT_THREAD) { return EXIT_THREAD; }
		}
	}
		...
		if (sn->FindPic(80, 506, 739, 539, _T("进度条.bmp"), _T("202020"), 0.9, 0, &intX, &intY) != -1) {
			if (DelayPlus(game, sn, 500, 800) == EXIT_THREAD) { return EXIT_THREAD; }
			sn->DragMouse(intX + 15, intY, intX + 165, intY);
			if (DelayPlus(game, sn, 500, 800) == EXIT_THREAD) { return EXIT_THREAD; }
		}
	}
		...
		if (sn->FindPic(80, 506, 739, 539, _T("进度条.bmp"), _T("202020"), 0.9, 0, &intX, &intY) != -1) {
			if (DelayPlus(game, sn, 500, 800) == EXIT_THREAD) { return EXIT_THREAD; }
			sn->DragMouse(intX + 15, intY, intX + 195, intY);
			if (DelayPlus(game, sn, 500, 800) == EXIT_THREAD) { return EXIT_THREAD; }
		}
	}
		...
		if (sn->FindPic(80, 506, 739, 539, _T("进度条.bmp"), _T("202020"), 0.9, 0, &intX, &intY) != -1) {
			if (DelayPlus(game, sn, 500, 800) == EXIT_THREAD) { return EXIT_THREAD; }
			sn->DragMouse(intX + 15, intY, intX + 225, intY);
			if (DelayPlus(game, sn, 500, 800) == EXIT_THREAD) { return EXIT_THREAD; }
		}
	}
		...
		if (sn->FindPic(80, 506, 739, 539, _T("进度条.bmp"), _T("202020"), 0.9, 0, &intX, &intY) != -1) {
			if (DelayPlus(game, sn, 500, 800) == EXIT_THREAD) { return EXIT_THREAD; }
			sn->DragMouse(intX + 15, intY, intX + 255, intY);
			if (DelayPlus(game, sn, 500, 800) == EXIT_THREAD) { return EXIT_THREAD; }
		}
	}
		...
		if (sn->FindPic(80, 506, 739, 539, _T("进度条.bmp"), _T("202020"), 0.9, 0, &intX, &intY) != -1) {
			if (DelayPlus(game, sn, 500, 800) == EXIT_THREAD) { return EXIT_THREAD; }
			sn->DragMouse(intX + 15, intY, intX + 285, intY);
			if (DelayPlus(game, sn, 500, 800) == EXIT_THREAD) { return EXIT_THREAD; }
		}
	}
		...
		if (sn->FindPic(80, 506, 739, 539, _T("进度条.bmp"), _T("202020"), 0.9, 0, &intX, &intY) != -1) {
			if (DelayPlus(game, sn, 500, 800) == EXIT_THREAD) { return EXIT_THREAD; }
			sn->DragMouse(intX + 15, intY, intX + 315, intY);
			if (DelayPlus(game, sn, 500, 800) == EXIT_THREAD) { return EXIT_THREAD; }
		}
	}
		...
		if (sn->FindPic(80, 506, 739, 539, _T("进度条.bmp"), _T("202020"), 0.9, 0, &intX, &intY) != -1) {
			if (DelayPlus(game, sn, 500, 800) == EXIT_THREAD) { return EXIT_THREAD; }
			sn->DragMouse(intX + 15, intY, intX + 345, intY);
			if (DelayPlus(game, sn, 500, 800) == EXIT_THREAD) { return EXIT_THREAD; }
		}
	}
		...
		if (sn->FindPic(80, 506, 739, 539, _T("进度条.bmp"), _T("202020"), 0.9, 0, &intX, &intY) != -1) {
			if (DelayPlus(game, sn, 500, 800) == EXIT_THREAD) { return EXIT_THREAD; }
			sn->DragMouse(intX + 15, intY, intX + 375, intY);
			if (DelayPlus(game, sn, 500, 800) == EXIT_THREAD) { return EXIT_THREAD; }
		}
	}
		...
		if (sn->FindPic(80, 506, 739, 539, _T("进度条.bmp"), _T("202020"), 0.9, 0, &intX, &intY) != -1) {
			if (DelayPlus(game, sn, 500, 800) == EXIT_THREAD) { return EXIT_THREAD; }
			sn->DragMouse(intX + 15, intY, intX + 405, intY);
			if (DelayPlus(game, sn, 500, 800) == EXIT_THREAD) { return EXIT_THREAD; }
		}
	}
		...
		if (sn->FindPic(80, 506, 739, 539, _T("进度条.bmp"), _T("202020"), 0.9, 0, &intX, &intY) != -1) {
			if (DelayPlus(game, sn, 500, 800) == EXIT_THREAD) { return EXIT_THREAD; }
			sn->DragMouse(intX + 15, intY, intX + 435, intY);
			if (DelayPlus(game, sn, 500, 800) == EXIT_THREAD) { return EXIT_THREAD; }
		}
	}
		...
		if (sn->FindPic(80, 506, 739, 539, _T("进度条.bmp"), _T("202020"), 0.9, 0, &intX, &intY) != -1) {
			if (DelayPlus(game, sn, 500, 800) == EXIT_THREAD) { return EXIT_THREAD; }
			sn->DragMouse(intX + 15, intY, intX + 465, intY);
			if (DelayPlus(game, sn, 500, 800) == EXIT_THREAD) { return EXIT_THREAD; }
		}
	}
		...
		if (sn->FindPic(80, 506, 739, 539, _T("进度条.bmp"), _T("202020"), 0.9, 0, &intX, &intY) != -1) {
			if (DelayPlus(game, sn, 500, 800) == EXIT_THREAD) { return EXIT_THREAD; }
			sn->DragMouse(intX + 15, intY, intX + 495, intY);
			if (DelayPlus(game, sn, 500, 800) == EXIT_THREAD) { return EXIT_THREAD; }
		}
	}
		...
		if (sn->FindPic(80, 506, 739, 539, _T("进度条.bmp"), _T("202020"), 0.9, 0, &intX, &intY) != -1) {
			if (DelayPlus(game, sn, 500, 800) == EXIT_THREAD) { return EXIT_THREAD; }
			sn->DragMouse(intX + 15, intY, intX + 525, intY);
			if (DelayPlus(game, sn, 500, 800) == EXIT_THREAD) { return EXIT_THREAD; }
		}
	}
		...
		if (sn->FindPic(80, 506, 739, 539, _T("进度条.bmp"), _T("202020"), 0.9, 0, &intX, &intY) != -1) {
			if (DelayPlus(game, sn, 500, 800) == EXIT_THREAD) { return EXIT_THREAD; }
			sn->DragMouse(intX + 15, intY, intX + 555, intY);
			if (DelayPlus(game, sn, 500, 800) == EXIT_THREAD) { return EXIT_THREAD; }
		}
	}
		...
		if (sn->FindPic(80, 506, 739, 539, _T("进度条.bmp"), _T("202020"), 0.9, 0, &intX, &intY) != -1) {
			if (DelayPlus(game, sn, 500, 800) == EXIT_THREAD) { return EXIT_THREAD; }
			sn->DragMouse(intX + 15, intY, intX + 585, intY);
			if (DelayPlus(game, sn, 500, 800) == EXIT_THREAD) { return EXIT_THREAD; }
		}
	}
		...
		if (sn->FindPic(80, 506, 739, 539, _T("进度条.bmp"), _T("202020"), 0.9, 0, &intX, &intY) != -1) {
			if (DelayPlus(game, sn, 500, 800) == EXIT_THREAD) { return EXIT_THREAD; }
			sn->DragMouse(intX + 15, intY, intX + 615, intY);
			if (DelayPlus(game, sn, 500, 800) == EXIT_THREAD) { return EXIT_THREAD; }
		}
	}
...
int BeginExplore::AutoReplaceMaterial(CString progressBar, CString materialKinds, snow *sn, Game *game) {
	...
	while (1) {
		...
		if ((allFlag == 0) && (sn->FindPic(352, 78, 466, 122, _T("候补出战.bmp"), _T("202020"), 0.9, 0, &intX, &intY) != -1))
		{
			Log(_T("选择置入候补式神...\r\n"), game);
			if (DelayPlus(game, sn, 500, 800) == EXIT_THREAD) { return EXIT_THREAD; }
			sn->MoveClick(intX + 245, intY + 112);
	...
	while (1) {
		...
		if (sn->FindPic(19, 467, 76, 513, _T("全部.bmp"), _T("202020"), 0.9, 0, &intX, &intY) != -1)
		{
			...
			if (sn->FindPic(103, 220, 151, 270, _T("N卡.bmp"), _T("202020"), 0.9, 0, &intX, &intY) == -1) {
				Log(_T("选择狗粮类型...\r\n"), game);
				if (DelayPlus(game, sn, 500, 800) == EXIT_THREAD) { return EXIT_THREAD; }
				sn->MoveClick(clickX + 5, clickY + 5);
				if (DelayPlus(game, sn, 1500, 2500) == EXIT_THREAD) { return EXIT_THREAD; }
			}
		}
		if ((sn->FindPic(103, 220, 151, 270, _T("N卡.bmp"), _T("202020"), 0.9, 0, &intX, &intY) != -1) || (sn->FindPic(25, 211, 73, 257, _T("素材.bmp"), _T("202020"), 0.9, 0, &intX, &intY) != -1))
		{
			break;
		}
		if (DelayPlus(game, sn, 500, 800) == EXIT_THREAD) { return EXIT_THREAD; }
	...
	while (1) {
		...
		//27,466,73,509,
		if ((sn->FindPic(27, 466, 73, 509, _T("N卡(已选).bmp"), _T("202020"), 0.9, 0, &intX, &intY) != -1) || (sn->FindPic(27, 466, 73, 509, _T("素材(已选).bmp"), _T("202020"), 0.9, 0, &intX, &intY) != -1))
		{
			break;
		}
		if (DelayPlus(game, sn, 500, 800) == EXIT_THREAD) { return EXIT_THREAD; }
	...
	while (1) {
		//自动拒绝悬赏
		if (AutoRejectXS(sn, game) == EXIT_THREAD) { return EXIT_THREAD; }
		if ((beginX < 735) && (sn->FindPic(beginX, 359, 863, 430, _T("出战.bmp"), _T("202020"), 0.9, 0, &intX, &intY) != -1)) {
			startX = intX, startY = intY;
			if (sn->FindPic(intX - 87, intY, intX - 20, intY + 100, _T("补.bmp"), _T("202020"), 0.85, 0, &intX, &intY) == -1) {
				Log(_T("加入出战列表...\r\n"), game);
				sn->MoveTo(startX + 9, startY + 7);
				Sleep(200);
				sn->LeftDown();
				Sleep(2500);
				sn->LeftUp();
			...
			if (sn->FindPic(80, 506, 739, 539, _T("进度条.bmp"), _T("202020"), 0.9, 0, &intX, &intY) != -1) {
				...
				//模拟真实鼠标轨迹
				sn->EnableRealMouse(1, 20, 30);
				sn->DragMouse(intX + 15, intY - 5, intX + 70, intY);
				beginX = 81;
				if (DelayPlus(game, sn, 500, 1000) == EXIT_THREAD) { return EXIT_THREAD; }
			}
		}
...
int BeginExplore::ReceiveInvite(int count, int collectbox, int mode, snow *sn, Game *game) {
	...
	while (1)
	{
		...
		if (sn->FindPic(0, 0, 55, 50, _T("退出(透明).bmp"), _T("202020"), 0.9, 0, &intX, &intY) != -1) {
			game->reSetWaitFlag(exbonus1, exbonus2);
			game->reSetWaitFlag(exteamer1, exteamer2);
			break;
		}
...
//探索单刷
int BeginExplore::SingleExplore(long hd, int round, int layer, int diff, CString jc, int speed, CString progressBar, CString materialKinds, int collectbox, int mode, snow *sn, Game *game) {
	if (sn == NULL)
	{
		return 0;
	}
	...
	while (count < round)
	{
		...
		count_str.Format(_T("%d"), count + 1);
		Log(_T("第") + count_str + _T("轮探索开始...\r\n"), game);
		if (EnterExplore(speed, progressBar, materialKinds, sn, game) == EXIT_THREAD) { return EXIT_THREAD; }
		count++;
		//领取奖励宝箱
		if (ExitExploreEvent(sn, game) == EXIT_THREAD) { return EXIT_THREAD; }
		if (DelayPlus(game, sn, 200, 500) == EXIT_THREAD) { return EXIT_THREAD; }
...
//智能单刷
int BeginExplore::AIExplore(long hd, int round, int layer, int diff, CString jc, int speed, CString progressBar, CString materialKinds, int collectbox, int mode, snow *sn, Game *game) {
	if (sn == NULL)
	{
		return 0;
	}
...
//组队邀请好友
int BeginExplore::inviteExplore(long hd, int round, int layer, CString jc, int preferArea, int speed, int collectbox, int mode, CString progressBar, CString materialKinds, snow *sn, Game *game) {
	if (sn == NULL)
	{
		return 0;
	}
...
int BeginExplore::ExploreReturnYard(snow *sn, Game *game) {
	...
	while (1) {
		...
		if (sn->FindPic(330, 304, 453, 349, _T("退队取消.bmp"), _T("202020"), 0.85, 0, &intX, &intY) != -1) {
			if (DelayPlus(game, sn, 500, 1500) == EXIT_THREAD) { return EXIT_THREAD; }
			CPoint pos = sn->RandomOffsetPos(intX, intY, 0, 5);
			sn->MoveClick(pos.x, pos.y);
			if (DelayPlus(game, sn, 500, 1500) == EXIT_THREAD) { return EXIT_THREAD; }
		...
		if (sn->FindPic(523, 286, 640, 320, _T("探索确认.bmp"), _T("202020"), 0.85, 0, &intX, &intY) != -1) {
			if (DelayPlus(game, sn, 500, 1500) == EXIT_THREAD) { return EXIT_THREAD; }
			CPoint pos = sn->RandomOffsetPos(intX, intY, 0, 5);
			sn->MoveClick(pos.x, pos.y);
			if (DelayPlus(game, sn, 500, 1500) == EXIT_THREAD) { return EXIT_THREAD; }
