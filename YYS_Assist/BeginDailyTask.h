...
public:
	...
	//	1.3.52 星之屿
	int IslandOfStarOnlySoftWind(snow *sn, Game *game);
	//1.42 选buff
	//1.4.12 抽象出方法
	...
	//1.62 根据场景执行任务
	int AccordingSceneRunOnlySoftWind(int scene, snow *sn, Game *game);
	//2.02 开始任务
	int StartSixPathOfDoorOnlySoftWind(long hd, int round, snow *sn, Game *game);
	//----日常功能----
	//进入副本
	...
	//契灵
	int Vestige(long hd, int round, snow *sn, Game *game);
	//整合开始
