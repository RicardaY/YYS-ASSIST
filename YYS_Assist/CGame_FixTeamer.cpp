...
BOOL CGame_FixTeamer::OnInitDialog()
{
	...
	if (!PathIsDirectory(_T("fixTeamer"))) {
		...
		for (int i = 0; i < 6; i++) {
			...
			if (PathFileExists(_T("fixTeamer\\teamer") + num_index + _T(".bmp"))) {
				CString image_dir = _T("fixTeamer\\teamer") + num_index + _T(".bmp");
				hbitmap = (HBITMAP)::LoadImage(NULL, image_dir, IMAGE_BITMAP, 0, 0, LR_DEFAULTCOLOR | LR_DEFAULTSIZE | LR_LOADFROMFILE);
				m_picImage[i]->SetBitmap(hbitmap);
			}
			else {
...
{	
	...
	{
	case 0: {sn->Capture(337, 148, 397, 166, _T("fixTeamer\\teamer1.bmp")); break; }
	case 1:{sn->Capture(556, 148, 616, 166, _T("fixTeamer\\teamer2.bmp")); break; }
	case 2:{sn->Capture(337, 220, 397, 238, _T("fixTeamer\\teamer3.bmp")); break; }
	case 3:{sn->Capture(556, 220, 616, 238, _T("fixTeamer\\teamer4.bmp")); break; }
	case 4:{sn->Capture(337, 292, 397, 310, _T("fixTeamer\\teamer5.bmp")); break; }
	case 5:{sn->Capture(556, 292, 616, 310, _T("fixTeamer\\teamer6.bmp")); break; }
	default:
		break;
	}
	if (sn != nullptr) {
