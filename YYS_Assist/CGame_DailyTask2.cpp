...
// CGame_DailyTask2 对话框
IMPLEMENT_DYNAMIC(CGame_DailyTask2, CDialog)
CGame_DailyTask2::CGame_DailyTask2(CWnd* pParent /*=nullptr*/)
	: CDialog(IDD_YaoQiFengYin_DLG, pParent)
{
}
CGame_DailyTask2::~CGame_DailyTask2()
{
}
void CGame_DailyTask2::DoDataExchange(CDataExchange* pDX)
{
...
BEGIN_MESSAGE_MAP(CGame_DailyTask2, CDialog)
	ON_BN_CLICKED(IDC_BTN_YQFY_ADDTOLIST, &CGame_DailyTask2::OnBnClickedBtnYqfy2Addtolist)
	ON_CBN_SELCHANGE(IDC_COMBO_YQFY_KINDS, &CGame_DailyTask2::OnSelchangeComboYqfyKinds)
	ON_WM_CTLCOLOR()
END_MESSAGE_MAP()
// CGame_DailyTask2 消息处理程序
BOOL CGame_DailyTask2::OnInitDialog()
{
	CDialog::OnInitDialog();
	// TODO:  在此添加额外的初始化
	std::vector<std::any> line;
	int preset = sPreset->ReadLine(line, 5, fname2);
	if (preset && line.size() == 2) {
		...
		case 1: {
			...
			m_sub.InsertString(1, _T("椒图(溅射版)"));
			m_sub.SetCurSel(0);
			break;
		}
		case 2: {
...
extern CGame2 *p;
void CGame_DailyTask2::OnBnClickedBtnYqfy2Addtolist()
{
	//获取参数
	UpdateData(true);
	//次数
...
void CGame_DailyTask2::OnSelchangeComboYqfyKinds()
{
...
HBRUSH CGame_DailyTask2::OnCtlColor(CDC* pDC, CWnd* pWnd, UINT nCtlColor)
{
