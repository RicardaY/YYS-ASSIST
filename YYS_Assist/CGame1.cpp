...
BEGIN_MESSAGE_MAP(CGame1, CDialog)
	ON_WM_CTLCOLOR()
	ON_BN_CLICKED(IDC_BTN_DELETE_SELECT_TASK, &CGame1::OnBnClickedBtnDeleteSelectTask1)
	ON_BN_CLICKED(IDC_BTN_CLEAR_TASK_LIST, &CGame1::OnBnClickedBtnClearTaskList1)
	ON_BN_CLICKED(IDC_BTN_START, &CGame1::OnBnClickedBtnStart1)
	ON_BN_CLICKED(IDC_BTN_SUSPEND, &CGame1::OnBnClickedBtnSuspend1)
	ON_BN_CLICKED(IDC_BTN_STOP, &CGame1::OnBnClickedBtnStop1)
	...
	ON_BN_CLICKED(IDC_BTN_UPDATE, &CGame1::OnBnClickedBtnUpdate)
END_MESSAGE_MAP()
// CGame1 消息处理程序
BOOL CGame1::OnInitDialog()
{
	...
	m_tab_right.AddPage(_T("独立功能"), &dependent_func, IDD_INDEPENDENT);
	m_tab_right.AddPage(_T("御魂预设"), &changeEquip, IDD_CHANGEEQUIP);
	m_tab_right.AddPage(_T("定时功能"), &timer, IDD_TIMER);
	m_tab_right.AddPage(_T("运行日志"), &runlog, IDD_RunLog); 
	m_tab_right.AddPage(_T("全局设置"), &global, IDD_GLOBAL);
	m_tab_right.AddPage(_T("使用说明"), &tips, IDD_TIPS);
	//m_tab_right.AddPage(_T("超鬼王"), &superking, IDD_SUPERKING);
	m_tab_right.AddPage(_T("截图队友"), &fixTeamer, IDD_FIXTEAMER);
	...
	int preset = sPreset->ReadLine(line, 12, fname1);
...
UINT CGame1::ThreadProc(LPVOID lpParameter) {
	//初始化当前线程com组件为MTA模式
	snow *sn = new snow;
	//设置字库
	sn->SetPath(g_path);
	...
	if (m_handle.IsEmpty())
	{
		...
		game1->setThreadStartState(false);
		game1->setThreadStopState(true);
		//解绑
		if (sn->IsBind(handle))
		{
			sn->UnBindWindow();
	...
	int simMachineType = game1->m_windowType.GetCurSel();
	//输出提示信息
	game1->log(_T("正在初始化...\r\n"));
	//雷电(最小化)
	if (simMachineType == 0) {
	...
	//定时启动
	int timingStart = game1->timer.m_tmstart.GetCheck();
	CString timingConstr;
	game1->timer.GetDlgItemText(IDC_GRTP_ROUND, timingConstr);
	int timingCon = StrToInt(timingConstr);
	if (timingStart) {
		int sleeptime = timingCon * 60;
		int i = 0;
		yys_function *waitfunc = new yys_function;
		game1->log(_T("定时启动开始，开始等待...\r\n"));
		while (i< sleeptime) {
			if (waitfunc->DelayPlus(game1, sn, 3000, 3002) == EXIT_THREAD) {
				game1->log(_T("线程已停止!\r\n"));
		...
		game1->log(_T("定时时间到，开始启动任务!\r\n"));
		if (waitfunc != nullptr) { delete waitfunc; waitfunc = nullptr; }
	}
	//遍历独立/定时功能列表
	int amount = game1->dependent_func.m_amount.GetCurSel();
	int nCount = game1->m_list_taskshow.GetItemCount();
	int baigui = game1->dependent_func.m_baigui.GetCheck();
	int regionKing = game1->dependent_func.m_king.GetCheck();
	int kindCheck = game1->dependent_func.GetCheckedRadioButton(IDC_RADIO_HOT, IDC_RADIO_COLLECT);//选中的索引
	int plot = game1->dependent_func.m_plot.GetCheck();
	int pvp = game1->dependent_func.m_pvp.GetCheck();
	int pvp_type = game1->dependent_func.m_pvptype.GetCurSel();
	int ready = game1->dependent_func.m_ready.GetCheck();
	int clearing = game1->dependent_func.m_clearing.GetCheck();
	int whitecard = game1->dependent_func.m_whitecard.GetCheck();
	int challenge = game1->dependent_func.m_challenge.GetCheck();
	int smallpve = game1->dependent_func.m_small.GetCheck();
	int lingran = game1->dependent_func.m_lingran.GetCheck();
	int dependFlag = baigui | regionKing | plot | pvp | ready | clearing | whitecard | challenge | smallpve | lingran;
	int adopt = game1->timer.m_adopt.GetCheck();
	...
	if (dependFlag == 1) {
		...
		if (baigui == 1) {
			int threadRes = dependFunc.StartBaiGui(1000, sn, game1);
			//中途结束线程
			...
			int threadRes = dependFunc.StartReginalKing(handle, amount, kindCheck, sn, game1);
			//中途结束线程
			if (threadRes == EXIT_THREAD)
			{
				game1->log(_T("线程已停止!\r\n"));
			...
			int threadRes = dependFunc.AutoPlot(sn, game1);
			//中途结束线程
			if (threadRes == EXIT_THREAD)
			{
				game1->log(_T("线程已停止!\r\n"));
			...
			int threadRes;
			if (pvp_type == 0) {
				threadRes = dependFunc.AutoPVP(sn, game1);
			}else if (pvp_type == 1) {
				threadRes = dependFunc.AutoPVPToMingShi(sn, game1);
			}
			//中途结束线程
			if (threadRes == EXIT_THREAD)
			{
			...
			int threadRes;
			threadRes = dependFunc.DIYFunction(ready, clearing, whitecard, challenge, sn, game1);
			//中途结束线程
			if (threadRes == EXIT_THREAD)
			{
				game1->log(_T("线程已停止!\r\n"));
			...
			int threadRes = dependFunc.StartMiWen(sn, game1);
			//中途结束线程
			if (threadRes == EXIT_THREAD)
			{
				game1->log(_T("线程已停止!\r\n"));
			...
			int threadRes = dependFunc.StartLingRan(sn, game1);
			//中途结束线程
			if (threadRes == EXIT_THREAD)
			{
				game1->log(_T("线程已停止!\r\n"));
	...
	for (int j = 0; j < circle; j++) {
		for (int i = 0; i < nCount; i++)
		{
			if (strResult[i].GetAt(0) == _T("探索")) {
				...
				if (mode == 1) {
					...
					if ((preferArea == 3) || (preferArea == 4)) {
						if (!PathIsDirectory(_T("fixTeamer"))) {
							game1->log(_T("未检测到固定队友图片目录，请先使用右侧功能-->截图固定队友！\r\n"));
							game1->log(_T("线程已停止！\r\n"));
							//解绑
							if (sn->IsBind(handle))
							{
				...
				//是否预设御魂
				if (game1->changeEquip.m_ispreset.GetCheck() == 1) {
					int group = game1->changeEquip.m_tsg.GetCurSel();
					int team = game1->changeEquip.m_tst.GetCurSel();
					BeginChangeYuHun changeYuHun;
					changeYuHun.StartChangeYuHun(handle,group, team,  sn, game1);
				}
				if (mode == 0)
				{
					threadRes = explore.BeinvitedExplore(handle, round, jc_str, reward, mode, sn, game1);
				}
				else if (mode == 1)
				{
					threadRes = explore.inviteExplore(handle, round, layer, jc_str, preferArea, speed, reward, mode, progressBar_str, materialKinds_str, sn, game1);
				}
				else if (mode == 2)
				{
					if (ai == 0) {
						threadRes = explore.SingleExplore(handle, round, layer, diff, jc_str, speed, progressBar_str, materialKinds_str, reward, mode, sn, game1);
					}
					else if (ai == 1) {
						threadRes = explore.AIExplore(handle, round, layer, diff, jc_str, speed, progressBar_str, materialKinds_str, reward, mode, sn, game1);
					}
				}
				//中途结束线程
				if (threadRes == EXIT_THREAD)
				...
				//是否预设御魂
				if (game1->changeEquip.m_ispreset.GetCheck() == 1) {
					int group = game1->changeEquip.m_yhg.GetCurSel();
					int team = game1->changeEquip.m_yht.GetCurSel();
					BeginChangeYuHun changeYuHun;
					changeYuHun.StartChangeYuHun(handle, group, team, sn, game1);
				}
				if (mode == (int)IDC_RADIO_YH_ASK)
				{
					threadRes = yuhun.inviteYuhun(handle, round, layer, jc_str, mateType, triple, sn, game1);
				}
				else if (mode == (int)IDC_RADIO_YH_BEASKED)
				{
					threadRes = yuhun.BeinvitedYuhun(handle, round, jc_str, sn, game1);
				}
				else if (mode == (int)IDC_RADIO_YH_SINGLE)
				{
					threadRes = yuhun.SingleYuHun(handle, round, layer, jc_str, sn, game1);
				}
				//中途结束线程
				if (threadRes == EXIT_THREAD)
				{
				...
				//是否预设御魂
				if (game1->changeEquip.m_ispreset.GetCheck() == 1) {
					int group = game1->changeEquip.m_jxg.GetCurSel();
					int team = game1->changeEquip.m_jxt.GetCurSel();
					BeginChangeYuHun changeYuHun;
					changeYuHun.StartChangeYuHun(handle, group, team, sn, game1);
				}
				if (monster_mode == (int)IDC_RADIO_JX_ASK)
				{
					threadRes = juexing.inviteJueXing(handle, round, mode, layer, jc_str, mateType, triple, sn, game1);
				}
				else if (monster_mode == (int)IDC_RADIO_JX_BEASKED)
				{
					threadRes = juexing.BeinvitedJueXing(handle, round, jc_str, sn, game1);
				}
				else if (monster_mode == (int)IDC_RADIO_JX_SINGLE)
				{
					threadRes = juexing.SingleJueXing(handle, round, mode, layer, jc_str, sn, game1);
				}
				//中途结束线程
				if (threadRes == EXIT_THREAD)
				{
				...
				//是否预设御魂
				if (game1->changeEquip.m_ispreset.GetCheck() == 1) {
					int group = game1->changeEquip.m_rlzyg.GetCurSel();
					int team = game1->changeEquip.m_rlzyt.GetCurSel();
					BeginChangeYuHun changeYuHun;
					changeYuHun.StartChangeYuHun(handle, group, team, sn, game1);
				}
				if (mode == (int)IDC_RADIO_RLZY_ASK)
				{
					threadRes = rilunzhiyun.inviteRiLunZhiYun(handle, round, layer, jc_str, mateType, triple, sn, game1);
				}
				else if (mode == (int)IDC_RADIO_RLZY_BEASKED)
				{
					threadRes = rilunzhiyun.BeinvitedRiLunZhiYun(handle, round, jc_str, sn, game1);
				}
				else if (mode == (int)IDC_RADIO_RLZY_SINGLE)
				{
					threadRes = rilunzhiyun.SingleRiLunZhiYun(handle, round, layer, jc_str, sn, game1);
				}
				//中途结束线程
				if (threadRes == EXIT_THREAD)
				{
				...
				//是否预设御魂
				if (game1->changeEquip.m_ispreset.GetCheck() == 1) {
					int group = game1->changeEquip.m_yszhg.GetCurSel();
					int team1 = game1->changeEquip.m_yszht1.GetCurSel();
					int team2 = game1->changeEquip.m_yszht2.GetCurSel();
					BeginChangeYuHun changeYuHun;
					changeYuHun.StartChangeYuHunDouble(handle, group, team1, team2, sn, game1);
				}
				if (mode == (int)IDC_RADIO_YSZH_ASK)
				{
					threadRes = yongshengzhihai.inviteYongShengZhiHai(handle, round, layer, jc_str, mateType, sn, game1);
				}
				else if (mode == (int)IDC_RADIO_YSZH_BEASKED)
				{
					threadRes = yongshengzhihai.BeinvitedYongShengZhiHai(handle, round, jc_str, sn, game1);
				}
				else if (mode == (int)IDC_RADIO_YSZH_SINGLE)
				{
					threadRes = yongshengzhihai.SingleYongShengZhiHai(handle, round, layer, jc_str, sn, game1);
				}
				//中途结束线程
				if (threadRes == EXIT_THREAD)
				{
			...
			BeginDailyTask dailyTask;
				...
				int threadRes;
				//是否预设御魂
				if ((monster_kind == 0)&&(game1->changeEquip.m_ispreset.GetCheck() == 1)) {
					int group = game1->changeEquip.m_yqfyg.GetCurSel();
					int team = game1->changeEquip.m_yqfyt.GetCurSel();
					BeginChangeYuHun changeYuHun;
					changeYuHun.StartChangeYuHun(handle, group, team, sn, game1);
				}
				threadRes = dailyTask.StartAllDaily(handle, condition, round, monster_kind, subsel, sn, game1);
				//中途结束线程
				if (threadRes == EXIT_THREAD)
				{
					game1->log(_T("线程已停止!\r\n"));
				...
				//是否预设御魂
				if (game1->changeEquip.m_ispreset.GetCheck() == 1) {
					int group = game1->changeEquip.m_yyhg.GetCurSel();
					int team = game1->changeEquip.m_yyht.GetCurSel();
					BeginChangeYuHun changeYuHun;
					changeYuHun.StartChangeYuHun(handle,group, team,  sn, game1);
				}
				threadRes = yeyuanhuo.StartYeYuanHuo(handle, round, monster_kind, sn, game1);
				//中途结束线程
				if (threadRes == EXIT_THREAD)
				{
					game1->log(_T("线程已停止!\r\n"));
				...
				//是否预设御魂
				if (game1->changeEquip.m_ispreset.GetCheck() == 1) {
					int group = game1->changeEquip.m_jjtpg.GetCurSel();
					int team = game1->changeEquip.m_jjtpt.GetCurSel();
					BeginChangeYuHun changeYuHun;
					changeYuHun.StartChangeYuHun(handle, group, team, sn, game1);
				}
				if (tpmode == 0) {
					if (mode == 0) {
						threadRes = tupo.StartTuPo(handle, round, sn, game1);
					}
					else if (mode == 1) {
						threadRes = tupo.StartTuPoByExitFourAttackNine(handle, round, sn, game1);
					}
				}
				else if (tpmode == 1) {
					threadRes = tupo.TeamBreak(handle, wait, sn, game1);
				}
				//中途结束线程
				if (threadRes == EXIT_THREAD)
				{
				...
				//是否预设御魂
				if (game1->changeEquip.m_ispreset.GetCheck() == 1) {
					int group = game1->changeEquip.m_ylg.GetCurSel();
					int team = game1->changeEquip.m_ylt.GetCurSel();
					BeginChangeYuHun changeYuHun;
					changeYuHun.StartChangeYuHun(handle, group, team, sn, game1);
				}
				threadRes = yuling.StartYuLing(handle, round, monster_kind, layer, sn, game1);
				//中途结束线程
				if (threadRes == EXIT_THREAD)
				{
					game1->log(_T("线程已停止!\r\n"));
				...
				//定时功能
				int adoptkind = game1->timer.m_kind.GetCurSel();
				int adoptstar = game1->timer.m_star.GetCurSel();
				int adoptarea = game1->timer.m_area.GetCurSel();
				int minite = StrToInt(minite_str);
				//控制线程结束标志
				int threadRes;
				threadRes = rest.StartRest(handle, minite, adopt, adoptkind, adoptstar, adoptarea, sn, game1);
				//中途结束线程
				if (threadRes == EXIT_THREAD)
				{
					game1->log(_T("线程已停止!\r\n"));
				...
				//控制线程结束标志
				int threadRes;
				threadRes = dependFunc.StartNewEvent(round, mode, flag, bossflag, sn, game1);
				//中途结束线程
				if (threadRes == EXIT_THREAD)
				{
					game1->log(_T("线程已停止!\r\n"));
				...
				//开始超鬼王
				int threadRes;
				threadRes = superghostking.StartSuperGhostKing(round, strenghCheck, group, team, blood, flag_own, flag_boss, star_stop, star_public, sn, game1);
				//中途结束线程
				if (threadRes == EXIT_THREAD)
				{
					game1->log(_T("线程已停止!\r\n"));
	...
	game1->log(_T("助手执行完毕!\r\n"));
	//微信推送
	int pushstate = game1->pushplus.m_push.GetCheck();
	CString token;
	game1->pushplus.GetDlgItemText(IDC_EDIT_TOKEN, token);
	yys_function yys;
	yys.PushPlus(token, pushstate, _T("Game1"), game1);
	game1->setThreadStopState(true);
	game1->setThreadStartState(false);
	
	return 1;
}
UINT CGame1::ThreadProcSaveFile(LPVOID lpParameter) {
	CGame1 *game1 = (CGame1*)lpParameter;
	SavePreSet preset;
	preset.SaveToFile(game1, game1->getFileName());
	return 0;
}
...
void CGame1::OnBnClickedBtnUpdate()
{
...
void CGame1::log(CString logMsg) {
	runlog.m_runLog.SetSel(0, 0, FALSE);
	runlog.m_runLog.ReplaceSel(GetTime() + logMsg, FALSE);
}
void CGame1::setThreadPauseState(bool state) {
	pause = state;
}
bool CGame1::getThreadPauseState() {
	return pause;
}
void CGame1::setThreadStartState(bool state) {
	start = state;
}
bool CGame1::getThreadStartState() {
	return start;
}
void CGame1::setThreadStopState(bool state) {
	stop = state;
}
bool CGame1::getThreadStopState() {
	return stop;
}
void CGame1::initThread() {
	pause = false;
	start = true;
	stop = false;
}
void CGame1::synchronization(bool &waitFlag1, bool &waitFlag2){
	int relation = global.m_relation.GetCheck();
	if (relation) {
		log(_T("开启了组队关系，等待队友同步...\r\n"));
		//通知事件2
		waitFlag2 = false;
		//等待事件1
		while (waitFlag1) {
...
//重置同步状态
void CGame1::reSetWaitFlag(bool &flag1, bool &flag2) {
	int relation = global.m_relation.GetCheck();
	if (relation) {
		flag1 = true;
	}
}
std::string CGame1::getFileName() {
	fileName = fname1;
	return fileName;
}
//保存预设
int CGame1::SaveToFile(std::string savePath){
