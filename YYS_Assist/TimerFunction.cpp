...
int TimerFunction::ClockAdopt(long hd, int adopt, int cardType, int star, int prior, snow *sn, Game *game) {
	...
	while (1) {
		...
		if (sn->FindStrFast(798, 495, 850, 535, _T("结界"), _T("D0CCB8-282728"), 0.9, &intX, &intY) != -1) {//D0CCB8-282728
			if (DelayPlus(game, sn, 500, 800) == EXIT_THREAD) { return EXIT_THREAD; }
		...
		if (sn->FindPic(16, 460, 76, 519, _T("全部.bmp"), _T("202020"), 0.9, 0, &intX, &intY) != -1) {
			Log(_T("成功进入育成界面...\r\n"), game);
			break;
		}
	...
	while (1) {
		//自动拒绝悬赏
		if (AutoRejectXS(sn, game) == EXIT_THREAD) { return EXIT_THREAD; }
		if (sn->FindPic(792, 52, 835, 118, _T("寄养.bmp"), _T("202020"), 0.9, 0, &intX, &intY) != -1) {
			if (DelayPlus(game, sn, 500, 800) == EXIT_THREAD) { return EXIT_THREAD; }
			if (sn->FindPic(868, 51, 911, 90, _T("我的寄养.bmp"), _T("202020"), 0.9, 0, &intX, &intY) != -1) {
				if (DelayPlus(game, sn, 500, 800) == EXIT_THREAD) { return EXIT_THREAD; }
	...
	while (1) {
		...
		if (sn->FindStrFast(740, 41, 915, 85, _T("友"), _T("3C3A34-3C3A35"), 0.9, &intX, &intY) != -1) {
			if (DelayPlus(game, sn, 500, 800) == EXIT_THREAD) { return EXIT_THREAD; }
			sn->MoveClick(159, 439);
			Log(_T("选择第一个狗粮(素材)当作寄养...\r\n"), game);
			if (DelayPlus(game, sn, 500, 1000) == EXIT_THREAD) { return EXIT_THREAD; }
		}
		if (sn->FindStrFast(522, 389, 586, 424, _T("确定"), _T("4D3A21-4E3B22"), 0.9, &intX, &intY) != -1) {
			if (DelayPlus(game, sn, 500, 800) == EXIT_THREAD) { return EXIT_THREAD; }
