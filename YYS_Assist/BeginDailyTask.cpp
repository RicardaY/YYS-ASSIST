...
int BeginDailyTask::OpenSeaOfMoon(snow *sn, Game *game) {
	...
	while (1) {
		...
		if (sn->FindStrFast(539, 309, 599, 339, _T("确定"), _T("4D3A21-4E3B22"), 0.9, &intX, &intY) != -1) {
			if (DelayPlus(game, sn, 500, 800) == EXIT_THREAD) { return EXIT_THREAD; }
			Log(_T("确定消耗60体...\r\n"), game);
		...
		if (sn->FindStrFast(523, 301, 591, 332, _T("确认"), _T("4D3A21-4E3B22"), 0.9, &intX, &intY) != -1) {
			if (DelayPlus(game, sn, 500, 1500) == EXIT_THREAD) { return EXIT_THREAD; }
			Log(_T("确认跳过剧情...\r\n"), game);
...
int BeginDailyTask::Battle(snow *sn, Game *game) {
	...
	while (1) {
		...
		if (sn->FindPic(0, 0, 56, 58, _T("退出(透明2).bmp|退出(透明).bmp"), _T("202020"), 0.9, 0, &intX, &intY) != -1) {
			break;
		}
		if (sn->FindPic(832, 393, 941, 536, _T("准备.bmp|准备2.bmp"), _T("202020"), 0.85, 0, &intX, &intY) != -1) {
			break;
...
int BeginDailyTask::IslandOfStar(snow *sn, Game *game) {
	...
	while (1) {
		...
		if (sn->FindStrFast(511, 298, 601, 335, _T("确认"), _T("4D3A21-4E3B22"), 0.9, &intX, &intY) != -1) {
			if (DelayPlus(game, sn, 200, 500) == EXIT_THREAD) { return EXIT_THREAD; }
			sn->MoveClick(intX + 10, intY + 10);
			Log(_T("确认刷新...\r\n"), game);
			if (DelayPlus(game, sn, 500, 1000) == EXIT_THREAD) { return EXIT_THREAD; }
...
//	1.3.52 星之屿
int BeginDailyTask::IslandOfStarOnlySoftWind(snow *sn, Game *game) {
	if (sn == NULL)
	{
		return 0;
	}
...
int BeginDailyTask::BattleSelectBuffOnlySoftWind(snow *sn, Game *game) {
	...
	while (1) {
		...
		if (buffLoc_xy.x != 0) {
			...
			//--找其他(include softwind)
			buffLoc_xy = FindBuff(_T("柔风抱暖|六道暴虐"), sn, game);
			if (buffLoc_xy.x != 0) {
				if (sn->FindStrFast(buffLoc_xy.x - 40, 433, buffLoc_xy.x + 100, 467, _T("选择"), _T("CAC7B5-2E2C2B"), 0.85, &intX, &intY) != -1) {
					CPoint pos = sn->RandomOffsetPos(intX, intY, 1, 5);
					sn->MoveClick(pos.x, pos.y);
...
int BeginDailyTask::ToWardsTsukuyomiOnlySoftWind(snow *sn, Game *game) {
	...
	if (softwind < 4) {
		...
		while (1) {
			...
			if (sn->FindStrFast(537, 306, 601, 343, _T("确定"), _T("4D3A21-4E3B22"), 0.9, &intX, &intY) != -1) {
				Log(_T("确定放弃...\r\n"), game);
				if (DelayPlus(game, sn, 200, 300) == EXIT_THREAD) { return EXIT_THREAD; }
				CPoint pos = sn->RandomOffsetPos(intX, intY, 5, 10);
				sn->MoveClick(pos.x, pos.y);
...
//1.62 根据场景执行任务
int BeginDailyTask::AccordingSceneRunOnlySoftWind(int scene, snow *sn, Game *game) {
	if (sn == NULL)
	{
		return 0;
	}
	...
	//星之屿 
	if (scene == ISLANDOFSTAR) {
		if (IslandOfStarOnlySoftWind(sn, game) == EXIT_THREAD) { return EXIT_THREAD; }
	}
	//月读 
	if (scene == BOSS) {
...
//2.02 开始任务 正常阵容
int BeginDailyTask::StartSixPathOfDoorOnlySoftWind(long hd, int round, snow *sn, Game *game) {
	if (sn == NULL)
	{
		return 0;
	}
	...
	while (count < round) {
		...
		while (1) {
			...
			if ((scene = RadomSelectAndCheckEvent(sn, game)) == EXIT_THREAD) { return EXIT_THREAD; }
			//4.根据场景执行任务
			if ((scene_res = AccordingSceneRunOnlySoftWind(scene, sn, game)) == EXIT_THREAD) { return EXIT_THREAD; }
			//退出条件
			if (scene_res == BOSS) {
				scene_res = -1;
				break;
...
int BeginDailyTask::EnterTaskFunc(long hd, int condition, CString picName, CString fontName, int mode_index, snow *sn, Game *game) {
	...
	while (1) {
		...
		if (sn->FindStrFast(269, 25, 331, 68, _T("加成"), _T("5D5139-362D19"), 0.8, &intX, &intY) != -1)
		{
			Log(_T("排队中...\r\n"), game);
			break;
		}
...
//契灵
int BeginDailyTask::Vestige(long hd, int round, snow *sn, Game *game) {
	if (sn == NULL)
	{
		return 0;
	}
...
int BeginDailyTask::StartAllDaily(long hd, int condition, int round, int headsel, int subsel, snow *sn, Game *game) {
	...
	case 1: {
		...
		case 1: {
			if (StartSixPathOfDoorOnlySoftWind(hd, round, sn, game) == EXIT_THREAD) { return EXIT_THREAD; }
			break;
		}
		default:
			break;
	...
	case 3: {
		if (Vestige(hd, round, sn, game) == EXIT_THREAD) { return EXIT_THREAD; }
		break;
	}
	default:
		break;
