	...
	for (int j = 0; j < circle; j++) {
		for (int i = 0; i < nCount; i++)
		{
			if (strResult[i].GetAt(0) == _T("探索")) {
				...
				{
					threadRes = explore.inviteExplore(handle, round, layer, jc_str, preferArea, speed, reward, mode, material, progressBar_str, materialKinds_str, sn, game1);
				}
				else if (mode == 2)
				{
					if (ai == 0) {
						threadRes = explore.SingleExplore(handle, round, layer, diff, jc_str, speed, material, progressBar_str, materialKinds_str, reward, mode, sn, game1);
					}
					else if (ai == 1) {
						threadRes = explore.AIExplore(handle, round, layer, diff, jc_str, speed, material, progressBar_str, materialKinds_str, reward, mode, sn, game1);
					}
				}
				//中途结束线程
				if (threadRes == EXIT_THREAD)
